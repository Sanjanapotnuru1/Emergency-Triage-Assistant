...
def html_bullets(items):
    li_items = "".join(f"<li>{html.escape(str(item))}</li>" for item in items if str(item).strip())
    return f'<ul class="severity-bullets">{li_items}</ul>'
...
def build_detailed_severity_sections(severity_sections, patient):
    ...
    for title, content in severity_sections.items():
        ...
        if title == "Symptoms Reported":
            detailed[title] = html_bullets([
                f"Primary symptoms: {symptoms}",
                f"Reported duration: {duration}",
                f"Existing medical conditions: {conditions}",
                "Clinical note: Symptoms should be interpreted along with duration, intensity, and relevant history"
            ])
        elif title == "Pain Score":
            detailed[title] = html_bullets([
                f"Patient-reported pain level: {pain}/10",
                "Interpretation: This score reflects the current discomfort severity and should be monitored for escalation",
                "Supporting context: Persistent pain with associated symptoms may require faster medical review"
            ])
        elif title == "Clinical Interpretation":
            detailed[title] = html_bullets([
                f"Assessment summary: {content}",
                f"Relevant conditions: {conditions}",
                "Clinical note: Overall interpretation should be correlated with physical examination and vital signs"
            ])
        elif title == "Recommended Action":
            detailed[title] = html_bullets([
                f"Immediate recommendation: {content}",
                "Patient guidance: Follow the advised care pathway without delay if symptoms worsen",
                "Escalation advice: Seek urgent in-person evaluation for breathing issues, worsening pain, or reduced responsiveness"
            ])
        elif title == "Precautions":
            detailed[title] = html_bullets([
                f"Precautionary care: {content}",
                "Monitoring advice: Track temperature, hydration, pain trend, breathing comfort, and general weakness",
                f"Special consideration: Blood pressure reading: {bp}",
                f"Special consideration: Blood sugar level: {sugar}"
            ])
        elif title == "Warning Signs":
            detailed[title] = html_bullets([
                f"Emergency warning signs: {content}",
                "Action note: If any warning sign appears suddenly or becomes severe, seek emergency care immediately"
            ])
        elif title == "AI Confidence Level":
            detailed[title] = html_bullets([
                f"Model confidence: {content}",
                "Interpretation note: Confidence reflects pattern matching from reported inputs and does not replace physician diagnosis"
            ])
        else:
            detailed[title] = html_bullets([content])
        ...
        for i in range(0, len(detailed_items), 2):
            ...
            for j in range(2):
                if i + j < len(detailed_items):
                    title, content in [(detailed_items[i + j][0], detailed_items[i + j][1])]:
                        with cols[j]:
                            st.markdown(
                                f"""
                                <div class="streamlit-severity-card">
                                    ...
                                    <div class="streamlit-severity-body">{content}</div>
                                </div>
                                """,
                                unsafe_allow_html=True
                            )
