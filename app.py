...
<style>
:root {{
    --bg: {bg}; --card: {card}; --card-alt: {card_alt}; --soft-card: {soft_card}; --text: {text}; --muted: {muted}; --faint: {faint}; --border: {border}; --input-bg: {input_bg}; --primary: {primary}; --primary-hover: {primary_hover}; --success-bg: {success_bg}; --success-text: {success_text}; --warning-bg: {warning_bg}; --warning-text: {warning_text}; --danger-bg: {danger_bg}; --danger-text: {danger_text}; --info-bg: {info_bg}; --info-text: {info_text};
}}
.stApp {{ background: radial-gradient(circle at top right, rgba(37,99,235,0.10), transparent 24%), linear-gradient(180deg, var(--bg), var(--bg)); color: var(--text); font-family: 'Segoe UI', sans-serif; }}
...
.hero {{ background: linear-gradient(135deg, var(--primary), var(--primary-hover)); border-radius: 24px; padding: 2rem 2rem 1.8rem; margin-bottom: 1.2rem; box-shadow: 0 16px 34px rgba(37,99,235,0.24); text-align: center; }}
.hero-title {{ font-size: clamp(2rem, 2.8vw, 3rem); font-weight: 800; color: #ffffff; margin-bottom: 0.35rem; }}
.hero-subtitle {{ color: #dbeafe; font-size: 1rem; }}
.info-banner {{ background: var(--info-bg); color: var(--info-text); border: 1px solid var(--border); border-radius: 18px; padding: 0.95rem 1rem; margin-bottom: 1rem; font-size: 0.95rem; }}
.form-card, .result-card {{ background: var(--card); border-radius: 22px; padding: 1.35rem; border: 1px solid var(--border); box-shadow: 0 10px 30px rgba(2, 6, 23, 0.12); margin-bottom: 1rem; }}
.section-title {{ font-size: 1.35rem; font-weight: 800; color: var(--text); margin-bottom: 0.35rem; }}
.section-subtitle {{ color: var(--muted); font-size: 0.96rem; margin-bottom: 1rem; }}
.stTextInput input, .stTextArea textarea, .stNumberInput input, div[data-baseweb="select"] > div, .stMultiSelect div[data-baseweb="select"] > div {{ background: var(--input-bg) !important; color: var(--text) !important; border-radius: 14px !important; border: 1px solid var(--border) !important; }}
.stTextArea textarea {{ min-height: 130px; }}
label, .stMarkdown, .stSelectbox label, .stMultiSelect label, .stFileUploader label {{ color: var(--text) !important; font-weight: 600 !important; }}
textarea::placeholder, input::placeholder {{ color: var(--faint) !important; opacity: 1 !important; }}
.stButton > button, .stDownloadButton > button, .stForm button[kind="primary"] {{ width: 100%; min-height: 50px; border: 1px solid rgba(255,255,255,0.08) !important; border-radius: 14px !important; background: linear-gradient(135deg, #163d8f, #1d4ed8) !important; color: #ffffff !important; font-size: 1rem !important; font-weight: 700 !important; box-shadow: 0 10px 24px rgba(29,78,216,0.24); }}
.stButton > button span, .stDownloadButton > button span, .stForm button[kind="primary"] span {{ color: #ffffff !important; }}
section[data-testid="stFileUploader"] [data-testid="stFileUploaderDropzone"] {{ background: #111827 !important; border: 1px solid rgba(148,163,184,0.24) !important; border-radius: 16px !important; padding: 0.75rem !important; }}
section[data-testid="stFileUploader"] button {{ background: #1f2937 !important; color: #f8fafc !important; border: 1px solid rgba(148,163,184,0.24) !important; border-radius: 12px !important; }}
section[data-testid="stFileUploader"] button span, section[data-testid="stFileUploader"] small, section[data-testid="stFileUploader"] div, section[data-testid="stFileUploader"] span, section[data-testid="stFileUploader"] p {{ color: #d1d5db !important; }}
.patient-chip {{ background: var(--soft-card); border: 1px solid var(--border); border-radius: 14px; padding: 0.9rem 1rem; margin-bottom: 0.8rem; }}
.patient-chip-title {{ font-size: 0.8rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 0.3rem; font-weight: 700; }}
.patient-chip-value {{ font-size: 1rem; font-weight: 700; color: var(--text); }}
.result-header {{ display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem; margin-bottom: 1rem; }}
.result-eyebrow {{ color: var(--muted); text-transform: uppercase; letter-spacing: 0.08em; font-size: 0.82rem; font-weight: 800; margin-bottom: 0.35rem; }}
.result-title {{ font-size: clamp(1.8rem, 2.2vw, 2.35rem); font-weight: 800; color: var(--text); margin-bottom: 0.25rem; }}
.result-subtext {{ color: var(--muted); font-size: 0.97rem; }}
.priority-badge {{ padding: 0.8rem 1rem; border-radius: 999px; font-size: 0.92rem; font-weight: 800; white-space: nowrap; }}
.priority-high {{ background: var(--danger-bg); color: var(--danger-text); border: 1px solid rgba(239,68,68,0.22); }}
.priority-moderate {{ background: var(--warning-bg); color: var(--warning-text); border: 1px solid rgba(245,158,11,0.20); }}
.priority-low {{ background: var(--success-bg); color: var(--success-text); border: 1px solid rgba(34,197,94,0.20); }}
.medical-card {{ background: var(--card); border: 1px solid var(--border); border-radius: 18px; padding: 1.1rem; margin-bottom: 1rem; box-shadow: 0 10px 24px rgba(2, 6, 23, 0.10); }}
.medical-card h3 {{ margin-bottom: 0.8rem; font-size: 1.05rem; color: var(--text); }}
.summary-grid {{ display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0.85rem; }}
.summary-card {{ background: var(--card-alt); border: 1px solid var(--border); border-radius: 16px; padding: 1rem; }}
.summary-label {{ color: var(--muted); font-size: 0.82rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.35rem; }}
.summary-value {{ color: var(--text); font-size: 1rem; font-weight: 700; line-height: 1.6; }}
.status-strip {{ padding: 1rem; border-radius: 14px; font-weight: 700; line-height: 1.6; }}
.strip-high {{ background: var(--danger-bg); color: var(--danger-text); border: 1px solid rgba(239,68,68,0.22); }}
.strip-moderate {{ background: var(--warning-bg); color: var(--warning-text); border: 1px solid rgba(245,158,11,0.20); }}
.strip-low {{ background: var(--success-bg); color: var(--success-text); border: 1px solid rgba(34,197,94,0.20); }}
.notes-box {{ background: var(--card-alt); border: 1px solid var(--border); border-radius: 14px; padding: 1rem; color: var(--text); line-height: 1.8; white-space: pre-wrap; }}
.severity-grid {{ display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 0.85rem; }}
.severity-item {{ background: var(--card-alt); border: 1px solid var(--border); border-radius: 16px; padding: 1rem; }}
.severity-item-title {{ font-size: 0.82rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.05em; font-weight: 700; margin-bottom: 0.45rem; }}
.severity-item-body {{ color: var(--text); line-height: 1.8; font-size: 0.97rem; }}
.clean-list {{ margin: 0; padding-left: 1.1rem; color: var(--text); line-height: 1.8; }}
.clean-list li {{ margin-bottom: 0.45rem; }}
.notice-card {{ background: linear-gradient(180deg, var(--warning-bg), transparent); }}
.footer {{ text-align: center; margin-top: 1.5rem; color: var(--muted); padding: 0.75rem; font-size: 0.92rem; }}
@media (max-width: 768px) {{ .block-container {{ padding-inline: 1rem; }} .hero {{ padding: 1.3rem; border-radius: 18px; }} .form-card, .result-card {{ padding: 1rem; }} .result-header {{ flex-direction: column; align-items: stretch; }} .summary-grid, .severity-grid {{ grid-template-columns: 1fr; }} }}
</style>
""", unsafe_allow_html=True)
